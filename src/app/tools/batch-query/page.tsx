...
export default function BatchQueryPage() {
	...
	const handleConfirm = () => {
		...
		if (newAddresses.length > 0) {
			...
			if (isAppendMode) {
				setAddresses((prevAddresses) => [...prevAddresses, ...(newAddresses as unknown as Address[])])
			} else {
	...
	return (
		<div className="min-h-[calc(100vh-4rem)] bg-slate-950 px-4 py-8 text-slate-100 sm:px-6 sm:py-10">
			<main className="mx-auto w-full max-w-7xl rounded-2xl border border-white/10 bg-white/5 p-4 sm:p-6">
				<BatchQueryForm
				...
				<Dialog open={isDialogOpen} onOpenChange={onDialogOpenChange}>
					<DialogContent className="min-w-[700px] border-white/15 bg-slate-900 text-slate-100">
						<DialogHeader>
							<DialogTitle>{isAppendMode ? '追加地址' : '导入地址'}</DialogTitle>
							<DialogDescription>请在下方输入需要查询的地址，每行一个</DialogDescription>
						</DialogHeader>
						<div>
							<Textarea
								placeholder="请输入地址"
								className="min-h-[300px] border-white/15 bg-white/5 text-slate-100"
								value={textareaValue}
								onChange={handleTextareaChange}
							/>
						</div>
						<DialogFooter className="relative flex flex-col items-center">
							<Button variant="link" className="absolute left-0 pl-0 pr-0 underline text-slate-300">
